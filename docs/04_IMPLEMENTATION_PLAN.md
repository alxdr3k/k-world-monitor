...
| `PUB-1A.5` | P0-M6 | PUB | PUB-1A | **첫 publication (blog_long 1건)** — **카테고리 = 경제 + 주제 = "한국 부동산 시장/정책의 현주소, 리스크"** (DEC-009 lock, 좁히기는 진입 시점 Q-036 결정) × 1 Thesis × 1 blog_long ContentDraft → vault/publications/blog_long/ commit → git push → Cloudflare Pages build → 자체 사이트 publish. Substack / YouTube / X manual cross-post (cite footnote는 자체 사이트 URL anchor — AC-034 lint). **v0 turn-key MVP gate** | PUB-1A.4, AGG-1A.4, DEC-009, Q-036 resolution | AC-013, AC-018, AC-026, AC-028, AC-034, AC-035 | defined | planned | TBD | MVP gate accepted |
