...
| `INFRA-1B.7b` | P1+ | INFRA | INFRA-1B | **Research Orchestration Schema** (Q-049 + Q-050 + Q-051 통합 migration v8) — exploration_round / round_search_run / round_candidate_url / round_synthesis / research_turn / operator_context 6 신규 테이블 + document_fetch_state (Q-049) 신규 + research_session ALTER (title / mode / editorial_intent_id / total_cost_usd / max_rounds / per_session_cost_cap_usd / forked_from_session_id) + raw_cache_items.round_id + run_ledger.round_id / search_run_id / turn_id FK + Snapshot Neo4j property (raw_body_hash / canonical_text_hash / source_round_id) | INFRA-1B.7a, Q-049/050/051 resolution | (schema) | defined | planned | TBD | INFRA-1B.7c |
| `INFRA-1B.7c` | P1+ | INFRA | INFRA-1B | **Search Orchestrator** (Q-050) — `src/search/orchestrator.ts` — InternalEvidenceSearch (a/c) + ExternalAISearch (b) + KnownSourceRefresh + RoundSynthesizer (d) 4 layer 통합. RoundContextPack 생성 / 소비 | INFRA-1B.7b, DISCOVERY-EXT.1 | AC-028 | defined | planned | TBD | INFRA-1B.7d |
...
| `INFRA-1B.9` | P1+ | INFRA | INFRA-1B | **Canonical Text Hash** (Q-049) — Mozilla Readability + linkedom + normalize + sha256 = canonical_text_hash. Snapshot 에 raw_body_hash + canonical_text_hash 분리 (Snapshot.content_hash UNIQUE = raw only). 변경 감지 1차 = canonical diff, 2차 = raw diff | INFRA-1B.7b (schema), Q-049 resolution | AC-020 | defined | planned | TBD | INFRA-1B.10 |
| `INFRA-1B.10` | P0-M2 hardening | INFRA | INFRA-1B | **Article-Level Conditional Fetch + document_fetch_state** (Q-049 — GPT 권고 scope 확대) — article URL state table + If-None-Match / If-Modified-Since 전송 + 304 시 Snapshot 미생성 + state 만 갱신 + queue enqueue 전 fetch_state 검사. **P0-M2 게이트 검증 안에 흡수** (단순 + 즉시 비용 절감) | INFRA-1B.7b (schema, document_fetch_state migration), ADR-0028 | AC-003, AC-020 | defined | planned | TBD | INFRA-1B.8 |
...
| `AGG-1A.6` | P1+ | AGG | AGG-1A | **Multi-round Research Session** (Q-050) — `src/research/session/` — `pipeline research start \| round \| finalize` CLI + ResearchSession + ExplorationRound + parent_round_id branching + RoundContextPack propagation + ScenarioRevision lineage bridge + termination conditions enforcement. EditorialIntent.purpose enum (ADR-0025 갱신) + research_session.mode hybrid | INFRA-1B.7d, AGG-1A.5 EditorialIntent, RESEARCH-1A.2 | AC-014, AC-017 | defined | planned | TBD | EXTR-1A.7 |
