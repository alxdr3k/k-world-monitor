...
|---|---|---|---|---|---|---|
| `P0-M1` | Schema & Bulk Store Bootstrap — 10-stage ADR(0011 superseded by 0025; 0012-0021) lock + Neo4j Cypher schema + SQLite relational + R2 permitted-artifact 적용 | TBD | landed (gate 검증 대기) | AC-001, AC-002, AC-003, AC-005, AC-006, AC-008, AC-022, AC-023, AC-026, AC-032 | **M1 own slices**: INFRA-1A.1 ~ INFRA-1A.8 일괄 landed (migrations/neo4j/v1, migrations/sqlite/v1~v3, src/{domain,storage,utils}/, data/sources_seed.yaml, R2 policy, backup runbook). SPIKE-001 (Neo4j FTS p95 <1s) + AC-032 R2 upload audit ledger (Q-044) 미실시 — gate accept 차단 | M1 slice 전부 landed (INFRA-1A.* only). gate 검증 단계. SPIKE-001 결과 + Q-044 R2 audit code enforcement 후 accepted. (INFRA-1B.1 / 1B.1.x 는 P0-M2 phase slice — M2 evidence 로 분리, 본 row 에서 제외) |
| `P0-M2` | Source Registry & Collection Queue — Tier A seed (size cap 없음, v0 entry 72 source) + collectability_score + source policy gate + discovery → 큐 → fingerprint snapshot → chunk 1건 end-to-end | TBD | landed (gate 검증 대기) | AC-001, AC-009, AC-020, AC-022, AC-024 | **M2 own slices**: INFRA-1B.1 ~ INFRA-1B.5 일괄 landed (src/storage/source-registry/seed.ts, src/discovery/{fetch,parse,scheduler,worker}/, src/pipeline/access-intervention/, migrations/sqlite/v3~v6). M2 게이트 검증 (AC-022/023/024 evidence 확정) 미실시 | M1 의존. M2 own slice 전부 landed. **Cross-milestone early landings (참고만, 본 row gate 평가 제외)**: INFRA-1B.6 (P0-M3 slice, manual feedback CLI) + OPS-1A.1 (P0-M3 slice, run ledger) 가 M2 phase 안에서 early-landed — gate accept 는 각 milestone 행에서 별도 평가. P1-M2-hardening (1B.2.x/1B.3.x/1B.6.x) 은 P0-M6 accept 후 별도 진입 |
