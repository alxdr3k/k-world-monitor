...
| `P0-M6` | Thesis & Content & Cite Check — Thesis(stance + market_stance) → ContentDraft (v0 blog_long only, DEC-005) → Publication 1건 + cite check 5+1 + cascade + EvidencePack v0 4-section + **자체 사이트 Astro skeleton + 첫 발행** (ADR-0022, DEC-005, DEC-006) | TBD | planned | AC-013, AC-016, AC-018, AC-026, AC-028, **AC-034** | TBD | M5 의존 — **v0 turn-key MVP gate (운영자 발화 2주 박싱은 optimistic upper bound, 실제 2~6주 reality reflow 가능)**. AC-034 는 자체 사이트 publishing primary + 외부 cross-post canonical cite anchor lint (NFR-010 / REQ-027) |
...
- Q-021: Tier A source seed 30~50개 + source_perspective 분포 균형 (4 메타
  카테고리 분포로 reflow — DEC-004 + DEC-009 경제 우선). 30 source list
  proposed `docs/research/source-seed-list-2026-05.md`. 사용자 list review
  + accept 후 외부 repo `data/sources_seed.yaml` commit 시 resolved — INFRA-1A.6
  진입 전 lock
