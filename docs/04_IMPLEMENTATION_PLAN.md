...
| `P0-M6` | Thesis & Content & Cite Check — Thesis(stance + market_stance) → ContentDraft (v0 blog_long only, DEC-005) → Publication 1건 + cite check 5+1 + cascade + EvidencePack v0 4-section + **자체 사이트 Astro skeleton + 첫 발행** (ADR-0022, DEC-005, DEC-006) | TBD | planned | AC-013, AC-016, AC-018, AC-026, AC-028, **AC-034**, **AC-035** | TBD | M5 의존 — **v0 turn-key MVP gate (운영자 발화 2주 박싱은 optimistic upper bound, 실제 2~6주 reality reflow 가능)**. AC-034 = 외부 cross-post canonical cite anchor lint (NFR-010 / REQ-027). AC-035 = Astro Content Collection Zod schema build-time gate (REQ-027 build-fail 룰) |
