...
| `P1-MVP-prep` | **Production deploy readiness** — v0→production schema migration framework (ALTER-only contract, backfill scripts, drift CI check) + timestamp 형식 CHECK constraints. **P0-M6(MVP gate) accept 직전에 진입** (production deploy 전 마지막 hardening 묶음) | pre-deploy (P0-M6 직전) | planned | AC-032 (extended) | DEPLOY-1A.1 / DEPLOY-1A.2 | M6 deploy 전제. Q-040, Q-041 resolution. PR #26 codex review 컨텍스트 + PR #25 retro F-17. |
