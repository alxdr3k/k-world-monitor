...
| `P0-M2-hardening` | **P0-M2 post-MVP hardening sub-phase** — INFRA-1B.* slices 가 일괄 landed 된 후 P0-M2 게이트 검증 단계 안에서 진행되는 hardening slices 의 묶음 (NOT a separate milestone — accept 는 P0-M2 게이트 accept 와 동일). Landed (code hardening, 4 slices, all registered in slice 표): `INFRA-1B.3.x-audit` (PR #39 R2 upload audit ledger AC-032 / NFR-008) + `INFRA-1B.3.h1-policy-fix` (PR #41 R2 cross-source archive_policy guard, AI-P0-1 legal-safety P0) + `INFRA-1B.1.h1-source-bootstrap-neo4j` (PR #44 Neo4j Source node bootstrap + 3-way preflight + `BootstrapPreflightError` fail-fast, AI-P1-2 — discovery → snapshot E2E blocker 해소) + **`INFRA-1B.3.h2-queue-cli` (PR #45 `bun run discovery:process-queue` CLI + new-path `source_not_found_in_graph` TypedQueueError 통일 + `parseArgs` allowlist fail-fast, AI-P1-3 — operator E2E processing baseline)**. Landed (doc baseline, 3 PRs — sub-phase support, NOT code hardening): #42 (AI-P1-8 frontmatter YAML parse fix), #43 (AI-P1-9 `DOC-SYNC-2026-05-15`), **본 PR `PR-canonical-register-2026-05-15` (AI-P0-1 + AI-P1-2 + AI-P1-3 canonical slice 표 row 일괄 등록 + Milestones row landed list 갱신, 코드 변경 0건)**. Planned: `INFRA-1B.10` (Article-Level Conditional Fetch + document_fetch_state v7), `INFRA-1B.4.h1-chunker-policy-gate` (chunker archive_policy gate, AI-P1-1 / Q-053 D2), `INFRA-1B.1.h2-source-profile` (Source profile canonical store, AI-P1-4 / Q-054 D3 + Q-058 D7), `OPS-1B.h1-runtime-invariant-scanner` (R2 invariant scan, AI-P1-6), `INFRA-1B.3.h3-audit-hardening` (v8 trigger + upload_attempt_id, AI-P1-7), `DEPLOY-1A.0-migration-validation` (migrate dry-run validation, AI-P1-5), `INFRA-1A.9-validator-extension` (TRACE-041, AI-P2-5), `INFRA-1A.10-glossary-backfill` (AI-P2-5), `INFRA-1B.2.h1-discovery-hardening` (AI-P2-2/8), `INFRA-1A.h1-supply-chain-audit` (AI-P2-10), `INFRA-1B.2.h2-end-to-end-test` (AI-P2-9). | P0-M2 게이트 accept 와 동시 | in_progress (4 code hardening landed: #39 / #41 / #44 / #45 ; +3 doc baseline #42 / #43 / `PR-canonical-register-2026-05-15` ; 나머지 planned) | (P0-M2 row 와 동일 — independent gate 아님) | post-AI-P1-8 (PR #42) baseline 위에서 진행 | DEC-024 D8 attention-budget rule (P0/P1 only) 적용. 본 sub-phase 의 의의 = INFRA-1B.* MVP slices 가 landed 된 후 발견된 hardening 항목들의 묶음 anchor — P0-M2 gate accept 의 obligatory evidence 는 INFRA-1B.3.x-audit + INFRA-1B.3.h1-policy-fix + INFRA-1B.1.h1-source-bootstrap-neo4j (PR #44) + **INFRA-1B.3.h2-queue-cli (PR #45 — operator E2E processing baseline)** + AC-022/023/024 evidence 확정 + SPIKE-001 결과. 그 외는 P0-M2 gate 와 별도 trace 만. **"landed" 분리 의미**: code hardening = 실제 src/ + migrations/ + tests/ 변경으로 P0-M2 hardening 가설 (audit trail / cross-source policy / E2E executable / typed error unification) 을 충족; doc baseline = invariant validator + thin-doc 정합 + canonical ledger sync 회복으로 code hardening 의 후속 review 가능 상태 유지. 두 종류 모두 sub-phase 의 일부이나 gate accept 의 obligatory evidence 는 code hardening 만 보유. |
