...
| `P0-M2-hardening` | **P0-M2 post-MVP hardening sub-phase** — INFRA-1B.* slices 가 일괄 landed 된 후 P0-M2 게이트 검증 단계 안에서 진행되는 hardening slices 의 묶음 (NOT a separate milestone — accept 는 P0-M2 게이트 accept 와 동일). Landed (code hardening, 3 slices): `INFRA-1B.3.x-audit` (PR #39 R2 upload audit ledger AC-032 / NFR-008) + `INFRA-1B.3.h1-policy-fix` (PR #41 R2 cross-source archive_policy guard, AI-P0-1 legal-safety P0) + **`INFRA-1B.1.h1-source-bootstrap-neo4j` (PR #44 Neo4j Source node bootstrap + 3-way preflight + `BootstrapPreflightError` fail-fast — `src/storage/source-registry/neo4j-bootstrap.ts` + `scripts/seed-sources.ts --neo4j` / `--preflight` + `tests/unit/neo4j_bootstrap_test.ts` 23 tests, AI-P1-2 — discovery → snapshot E2E blocker 해소)**. Landed (doc baseline, 2 PRs — sub-phase support, NOT code hardening): #42 (AI-P1-8 frontmatter YAML parse fix — invariant validator 6 docs 회복) + #43 (AI-P1-9 `DOC-SYNC-2026-05-15` — current-state + IMPL_PLAN + HLD + PRD + SPIKES + TESTING 동기화). Planned: `INFRA-1B.10` (Article-Level Conditional Fetch + document_fetch_state v7), `INFRA-1B.3.h2-queue-cli` (queue CLI + typed `source_not_found_in_graph` error 통일, AI-P1-3), `INFRA-1B.4.h1-chunker-policy-gate` (chunker archive_policy gate, AI-P1-1 / Q-053 D2), `INFRA-1B.1.h2-source-profile` (Source profile canonical store, AI-P1-4 / Q-054 D3 + Q-058 D7), `OPS-1B.h1-runtime-invariant-scanner` (R2 invariant scan, AI-P1-6), `INFRA-1B.3.h3-audit-hardening` (v8 trigger + upload_attempt_id, AI-P1-7), `DEPLOY-1A.0-migration-validation` (migrate dry-run validation, AI-P1-5), `INFRA-1A.9-validator-extension` (TRACE-041, AI-P2-5), `INFRA-1A.10-glossary-backfill` (AI-P2-5), `INFRA-1B.2.h1-discovery-hardening` (AI-P2-2/8), `INFRA-1A.h1-supply-chain-audit` (AI-P2-10), `INFRA-1B.2.h2-end-to-end-test` (AI-P2-9). Canonical slice 표 row 추가는 별도 PR (`PR-canonical-register-2026-05-15` 가칭) 에서 진행 — 본 row 는 sub-phase anchor 만 등록 | P0-M2 게이트 accept 와 동시 | in_progress (3 code hardening landed: #39 / #41 / #44 ; +2 doc baseline #42 / #43 ; 나머지 planned) | (P0-M2 row 와 동일 — independent gate 아님) | post-AI-P1-8 (PR #42) baseline 위에서 진행 | DEC-024 D8 attention-budget rule (P0/P1 only) 적용. 본 sub-phase 의 의의 = INFRA-1B.* MVP slices 가 landed 된 후 발견된 hardening 항목들의 묶음 anchor — P0-M2 gate accept 의 obligatory evidence 는 INFRA-1B.3.x-audit + INFRA-1B.3.h1-policy-fix + **INFRA-1B.1.h1-source-bootstrap-neo4j (PR #44 — discovery E2E 실행 가능 baseline)** + AC-022/023/024 evidence 확정 + SPIKE-001 결과. 그 외는 P0-M2 gate 와 별도 trace 만. **"landed" 분리 의미**: code hardening = 실제 src/ + migrations/ + tests/ 변경으로 P0-M2 hardening 가설 (audit trail / cross-source policy / E2E executable) 을 충족; doc baseline = invariant validator + thin-doc 정합 회복으로 code hardening 의 후속 review 가능 상태 유지. 두 종류 모두 sub-phase 의 일부이나 gate accept 의 obligatory evidence 는 code hardening 만 보유. |
