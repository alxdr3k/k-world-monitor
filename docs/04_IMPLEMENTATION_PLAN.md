...
| `INFRA-1B.5` | P0-M2 | INFRA | INFRA-1B | access_interventions Neo4j 노드 + severity deterministic 산정 + batch_report mode 구현 | INFRA-1A.2, INFRA-1B.1 | AC-024 | defined | in_progress | PR #21 open (claude/infra-1b5-access-interventions → main) | INFRA-1B.6 |
| `INFRA-1B.6` | P0-M3 | INFRA | INFRA-1B | Manual feedback CLI — `pipeline feedback add|bulk|link|from-report` + `pipeline intervention review <id>` 3-option | INFRA-1B.5 | AC-025 | defined | in_progress | PR #22 open (claude/infra-1b6-feedback-cli → claude/infra-1b5-access-interventions) | EXTR-1A.1 |
