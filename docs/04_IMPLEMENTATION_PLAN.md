...
| `INFRA-1B.9` | P1+ | INFRA | INFRA-1B | **Canonical Text Hash** (Q-049 / DEC-021) — Mozilla Readability + linkedom + normalize + sha256 = canonical_text_hash. **Migration sequence (DEC-021 #3 구현 의무)**: (1) 기존 Snapshot.content_hash → raw_body_hash backfill 먼저 → (2) DROP snap_content_hash_unique + CREATE snap_raw_body_hash_unique → (3) canonical_text_hash nullable property + non-unique INDEX 추가 → (4) 신규 Snapshot Readability 적용 → (5) 기존 Snapshot canonical_text_hash 별도 backfill batch (host concurrency cap + daily candidate cap 와 분리) + document_fetch_state ALTER (canonical_text_hash 컬럼 추가). 변경 감지 1차 = canonical diff (ADR-0010 INV-0010-4 lock), 2차 = raw diff (source_changed_minor alert, v1+) | INFRA-1B.10 (document_fetch_state schema), DEC-021 (Q-049 resolution) | AC-020 | defined | planned | TBD | INFRA-1B.8 |
