...
- ~~Q-042 ~ Q-048~~: **resolved by DEC-020** (NFR-003 5-hop trace path
  ...
  required check + branch protection). Implement slices (note: 일부 신규
  slice 는 본 plan 의 slice 표에 아직 row 미등록 — TRACE-040/041 frontmatter
  anchor 만 존재, 슬라이스 entry condition 도달 시 slice 표에 row 추가
  의무): AGG-1A.1 (evidence_role 자동 검증, slice 표 등록) + 신규 slice
  Q-044 R2 audit (TRACE-040 anchor `src/storage/audit/policy-decisions.ts`
  + IntendedAction enum, INFRA-1B.3 의 follow-up 으로 구현 — slice ID 는
  미정, 진입 직전 lock) + OPS-1A.2 (quota module, slice 표 등록) + 신규
  slice invariant validator extension (TRACE-041 anchor, INFRA-1A 의
  follow-up — slice ID 미정) + CI admin task (운영자 admin step, code
  변경 없음)
