...
| `RESEARCH-1A.1` | P1+ | INFRA | RESEARCH-1A | **HTTP API + ResearchTurn schema + Auth** (Q-051 round 2 — web pivot) — `src/api/research-ask.ts` (Hono routes 가정, Q-051 stack 결정 후 확정) + research_turn 테이블 + operator_context 테이블 + flag routing (session / round / new-session / new-round / branch-from / fork-session / **answer-only**) + **Cloudflare Tunnel + Access wiring (v1+, Q-051 phasing lock)** + AI intent classifier 보조 (advisory only — auto-route 없음). CLI 는 v1+ optional thin wrapper | INFRA-1B.7b (schema), AGG-1A.6 (multi-round backend logic), Q-051 stack 결정 | AC-025 (확장) | defined | planned | TBD | RESEARCH-1A.2 |
