...
| `RESEARCH-1A.1` | P1+ | INFRA | RESEARCH-1A | **HTTP API + ResearchTurn schema + Auth** (Q-051 round 2 — web pivot) — `src/api/research-ask.ts` (Hono routes 가정, Q-051 stack 결정 후 확정) + research_turn 테이블 + operator_context 테이블 + **CLI flags**: `--session <id>` / `--round <id>` (context selection — route enum 은 `continue_round`) + `--new-session` (route=`new_session`) / `--new-round` (route=`new_round`) / `--branch-from <id>` (route=`branch_round`) / `--fork-session --from-session <id>` (route=`fork_session`) / `--answer-only` (route=`answer_only`). 7 flag = 6 route enum value (--session 과 --round 는 같은 `continue_round` 의 context selector). **Cloudflare Tunnel + Access wiring (v1+, Q-051 phasing lock)** + AI intent classifier 보조 (advisory only — auto-route 없음). CLI 는 P0-M6 critical path (PUB-1A.5 publish/edit), P1+ 부터 web /ops 가 primary, CLI 는 자동화/cron secondary | INFRA-1B.7b (schema), AGG-1A.6 (multi-round backend logic), Q-051 stack 결정 | AC-025 (확장) | defined | planned | TBD | RESEARCH-1A.2 |
| `RESEARCH-1A.2` | P1+ | INFRA | RESEARCH-1A | **Web UI core pages (mobile-first)** (Q-051) — `/ops` 홈 + `/ops/ask` global ask + `/ops/sessions` list + `/ops/sessions/:id` round timeline + `/ops/turns/:tid` + active context badge + Route dropdown (canonical enum: `continue_round` / `new_round` / `branch_round` / `fork_session` / `new_session` / `answer_only` — 6 route value, schema CHECK constraint 와 일치) + Tailwind responsive. Topology B `/ops/*` private prefix lock. UI-spec docs/design/ui-spec-research-app.md 참조 | RESEARCH-1A.1 | AC-025 (확장) | defined | planned | TBD | RESEARCH-1A.3 |
