...
| `PUB-1A.4` | P0-M6 | PUB | PUB-1A | **자체 사이트 Astro skeleton** — Astro 5.0 + Content Collection(`glob('vault/publications/**/*.{md,mdx}')`) + Zod schema mirror (status / cite_refs[] / correction_ledger[] / format) + `<Cite/>` / `<RetractionBanner/>` / `<CorrectionLedger/>` 컴포넌트 + pagefind + vega-lite/mermaid + `@astrojs/rss` 4 format feed + **Cloudflare Pages 배포** (git push trigger) (ADR-0022) | PUB-1A.3 | AC-018, NFR-010 | defined | planned | TBD | PUB-1A.5 |
| `PUB-1A.5` | P0-M6 | PUB | PUB-1A | **첫 publication (blog_long 1건)** — 1 카테고리 (DEC-004 4 메타 중 1개 선택, Q-031 별도 결정) × 1 Thesis × 1 blog_long ContentDraft → vault/publications/blog_long/ commit → git push → Cloudflare Pages build → 자체 사이트 publish. Substack / YouTube / X manual cross-post (cite footnote는 자체 사이트 URL anchor). **v0 turn-key MVP gate** | PUB-1A.4, AGG-1A.4 | AC-013, AC-018, AC-026, AC-028, NFR-010 | defined | planned | TBD | MVP gate accepted |
...
- Q-021: Tier A source seed 30~50개 + source_perspective 분포 균형 (4 메타
  카테고리 분포로 reflow — DEC-004) — INFRA-1A.6 진입 전 lock
