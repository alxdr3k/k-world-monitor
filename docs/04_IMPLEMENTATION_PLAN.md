...
| `INFRA-1B.1` | P0-M2 | INFRA | INFRA-1B | Source Registry Bootstrap — seed 72 sources from data/sources_seed.yaml into SQLite source_material_policy + Neo4j Source nodes + AC-022 validation. (Collection Queue, manual_intake CLI, 8 위험 행동 트리거는 INFRA-1B.5/1B.6으로 분리) | INFRA-1A.2, INFRA-1A.6 | AC-001, AC-022, AC-023 | defined | landed | src/storage/source-registry/seed.ts (parse+validate+upsert); scripts/seed-sources.ts (CLI --dry-run); tests/unit/source_registry_test.ts (12 tests); PR TBD | INFRA-1B.2 |
