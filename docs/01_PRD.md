...
| REQ-028 | (proposed — Q-043 결정 대기) Claim → Thesis 후보 link 의 `:EVIDENCE_FOR` relationship 에 `evidence_role` 필드 (6 enum: supporting / opposing / mitigating / amplifying / monitoring / context) + `assigned_by = operator_lock` 필수 (ADR-0027). 채택 시 REQ-023 본문 확장 옵션 (Q-043) 과 양자택일 — 본 row 는 사용자 결정 후 채택 / retract / REQ-023 흡수 중 하나 | proposed | AC-043, AC-044 | (Q-043) | ADR-0027 |
