...
| `scripts/check-vault-jsonl-policy.ts` | Pure policy validator for ADR-0012 INV-0012-5 (Markdown vault content kinds) + INV-0012-6 (JSONL not canonical store) — operator D4 2026-05-18. Exports `findVaultFiles` + `assertVaultContentKinds` (vault types must be one of Document hub / Dossier / Scenario / Thesis / ContentDraft / Publication / promoted claim) + `findJsonlFiles` + `assertJsonlIsNotCanonical` (every *.jsonl must be under allowlisted intermediate/log paths — `.dev-cycle/` / `docs/audit-export/` / `docs/_generated/` / `tests/fixtures/` family) + `checkVaultJsonlPolicy` aggregate. CLI entry: `bun run scripts/check-vault-jsonl-policy.ts [repoRoot]`. Vacuously true for vault while no vault root exists; JSONL guard is active over the live tree. |
...
| `tests/policy/vault_jsonl_policy_test.ts` | ADR-0012 INV-0012-5 (Markdown vault content kinds) + INV-0012-6 (JSONL not canonical) pure policy validator tests (32 tests). Live integration (4) + assertVaultContentKinds positive/negative (8) + assertJsonlIsNotCanonical allowlist/violation (12) + filesystem round-trip via tmpdir (8). Cycle 35 operator D4 2026-05-18. | (ADR-0012 vault / JSONL canonical-store guard) |
