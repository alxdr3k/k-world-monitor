...
> Last verified against code: 0a76d31 (code baseline, 2026-05-13 — last code-touching commit on branch: `src/discovery/worker/run-discovery.ts`, `src/storage/r2/client.ts`, `.github/workflows/{ci,doc-freshness,invariant-check}.yml`). Thin-doc edits since: 5aa70ac → ceaa17c → 38b846d → c7b9088 (PR #37 squash-merged to main as 18abf8f) → fa90659 (PR #38 round 0, Source Registry seam clarification) → this commit (2026-05-14, PR #38 Codex round 1 review fix: P1 Neo4j Source bootstrap claim 정정 — `src/discovery/worker/snapshot-fingerprint.ts` P1-3 guard 가 Source 부재 시 `source_not_found_in_graph` throw 하므로 \"discovery worker 가 Source 노드를 create / merge\" 라는 이전 표현은 코드 사실과 불일치, 후속 슬라이스 또는 운영자 manual Cypher 필요로 수정 + P2 `policy_decisions` 컬럼 목록을 v1 schema 실제 컬럼 (`decision_id` / `source_id` nullable / `session_id` nullable / `url` / `trigger_type` / `policy_gate_mode` CHECK / `decision` / `rationale` / `created_at`) 으로 align (planned `intended_action` / `risk_level` / `intervention_id` 는 INFRA-1B.3.x-audit 슬라이스 ALTER 예정 형태로 reflow) + P2 FK seam contract 에서 `source_policy_rules.applies_to_field` 가 source-id 가 아닌 policy 필드 enum 임을 명시 + P2 HLD diagram 의 blanket `FK src_id` 라벨을 per-table 분리 (`source_registry_slug_map` 은 value column, `source_policy_rules` 는 source-id 컬럼 부재, `policy_decisions` 는 nullable)). **코드 baseline (0a76d31) 이후 본 branch 의 모든 commit 은 docs-only** (verified via `git log --name-only 0a76d31..HEAD -- 'src/**' 'migrations/**' 'scripts/**' 'tests/**' '.github/**' 'package.json' 'tsconfig.json'` 빈 결과). 다음 코드 변경 시점에 baseline SHA 갱신.
...
3. Neo4j `Source` 노드 생성은 **현재 코드 미구현** — `src/discovery/worker/snapshot-fingerprint.ts` 의 `createDocumentAndSnapshot()` 안 P1-3 guard 가 `MATCH (src:Source {source_id: $sourceId})` 결과 0건이면 `Source not found in graph: <src_id>` 로 throw, 큐 row 는 `error_code='source_not_found_in_graph'` 로 마크된다 (v6 `discovery_queue.error_code` enum). 따라서 디스커버리 실행 전 Neo4j Source 노드는 별도 path 로 bootstrap 되어 있어야 한다 — 후속 슬라이스 (INFRA-1B.2+ 안에서 추가 예정, 현재 slice 표 미등록) 또는 운영자 manual Cypher 가 필요. `seedSources()` 가 SQLite 쪽 슬러그 매핑만 채우고 Neo4j 측 노드는 채우지 않는다는 사실은 idempotent dry-run 운용 의도이지 design oversight 가 아님 (DEC-015).
...
- **logical Source FK** 가 있는 컬럼: `source_material_policy.source_id` (NOT NULL, ADR-0017) / `source_registry_slug_map.source_id` (NOT NULL value column — slug→src_id mapping 의 결과값으로, constraint-level FK 가 아닌 lookup index, DEC-015 v3 migration) / `policy_decisions.source_id` (nullable — v1 schema line 89, 등록되지 않은 URL 에 대해서도 결정 기록 가능하도록 nullable). 모두 Neo4j `Source.src_id` 를 reference 한다. **DB-level FK constraint 로 enforce 되지 않음** (SQLite 안에서 cross-store FK 불가) — 정합성은 `seedSources()` 가 slug map 에서 `src_<ULID>` 를 받은 slug 에 대해서만 policy row 를 INSERT 한다는 idempotency contract (DEC-015) 로 유지.
