...
> Last verified against code: 0a76d31 (code baseline, 2026-05-13 — last code-touching commit on branch: `src/discovery/worker/run-discovery.ts`, `src/storage/r2/client.ts`, `.github/workflows/{ci,doc-freshness,invariant-check}.yml`). Thin-doc edits since: 5aa70ac → ceaa17c → 38b846d → c7b9088 (PR #37 squash-merged to main as 18abf8f) → fa90659 (PR #38 round 0, Source Registry seam clarification) → 2c629dc (PR #38 Codex round 1 fix) → c61a752 (PR #38 Codex round 2 fix) → this commit (2026-05-14, PR #38 Codex round 3 P2 fix: Neo4j `Source` node 의 composition table 속성 목록을 v0 landed schema (`migrations/neo4j/v1_schema.cypher` + `src/domain/nodes.ts` `SourceNode`) 와 align — `publisher_name` / `urls_root[]` / structured `collectability_score{...}` / `access_method` 는 HLD 계획안에만 있고 v0 미반영, 실제 landed = `source_id` PK + `name` + `url` + `reliability_tier` + 스칼라 `collectability_score` + `source_perspective` + denormalized policy 3 필드 + `created_at` 임을 명시). **코드 baseline (0a76d31) 이후 본 branch 의 모든 commit 은 docs-only** (verified via `git log --name-only 0a76d31..HEAD -- 'src/**' 'migrations/**' 'scripts/**' 'tests/**' '.github/**' 'package.json' 'tsconfig.json'` 빈 결과). 다음 코드 변경 시점에 baseline SHA 갱신.
