...
> Last verified against code: 0a76d31 (code baseline, 2026-05-13 — last code-touching commit on branch: `src/discovery/worker/run-discovery.ts`, `src/storage/r2/client.ts`, `.github/workflows/{ci,doc-freshness,invariant-check}.yml`). Thin-doc edits since: 5aa70ac → ceaa17c → 38b846d → c7b9088 (PR #37 squash-merged to main as 18abf8f) → fa90659 (PR #38 round 0, Source Registry seam clarification) → 2c629dc (PR #38 Codex round 1 fix) → this commit (2026-05-14, PR #38 Codex round 2 fix: P1 sources_seed.yaml header 의 \"Neo4j Source nodes follow in INFRA-1B.2+ discovery worker flow\" 잔존 표현 제거 + P2 FK seam contract / composition table 에서 Neo4j 의 canonical property 이름은 `source_id` (PK 값 형식만 `src_<ULID>`) 임을 명시 + P2 bootstrap step 3 의 error_code 표현 정확화 (dedup path 만 `source_not_found_in_graph`, new path 는 plain `Error` → `runtime_error`) + P2 HLD diagram 의 `src_id` 컬럼 라벨을 `source_id` 로 정정). **코드 baseline (0a76d31) 이후 본 branch 의 모든 commit 은 docs-only** (verified via `git log --name-only 0a76d31..HEAD -- 'src/**' 'migrations/**' 'scripts/**' 'tests/**' '.github/**' 'package.json' 'tsconfig.json'` 빈 결과). 다음 코드 변경 시점에 baseline SHA 갱신.
