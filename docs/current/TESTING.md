...
> Last verified against code: 3989b02 (2026-05-15) — AI-P1-12 / `INFRA-1B.5.h1-runbook-setup-hygiene` code commit on branch `claude/runbook-setup-hygiene`. Following commits are doc-only (TESTING SHA refresh) and do NOT advance the code baseline — this SHA is the latest CODE-touching commit on the branch. AI-P1-12 = RUNBOOK setup hygiene (Fresh worktree setup + GitHub access token revoke + Pre-commit hook env scrub) + new `scripts/check-secrets.ts` (pure `scanForSecrets()` 2-layer defense: filename guard + 8 vendor pattern matches) + `scripts/git-hooks/pre-commit` shim + `scripts/install-git-hooks.sh`; +24 tests in `check_secrets_test.ts` = 569 → 593 tests total. Tests breakdown: 6 filename guard (`isStagedEnvFile` / `isEnvFileExempt`) + 2 redaction format + 3 clean-files baseline + 3 env_file_staged + 8 per-vendor pattern (OpenAI / Anthropic / Google / AWS / GitHub PAT classic+fine-grained+oauth / Doppler) + 1 regex `g`-flag isolation + 1 redaction safety. Previous code baseline = (AI-P1-1 INFRA-1B.4.h1-chunker-policy-gate PR #47 squash — 561 → 569). Earlier baselines: 090ca5b (AI-P1-3 PR #45 — 544 → 561), 861796a (AI-P1-2 PR #44 — 521 → 544), 327f4b2 (AI-P0-1 PR #41 — 515 → 521), 75706c4 (INFRA-1B.3.x-audit PR #39 — 490 → 515). **Pre-merge SHA reachability**: branch-local commits (`3989b02`, ...) ARE reachable while the branch exists but will NOT survive squash-merge to main; the merge commit on main is the post-merge canonical reference and the next slice's baseline header advances to that SHA. For reproducibility while the PR is open, `git fetch origin <branch>` + `git checkout 3989b02` resolves correctly.
...
| unit tests | `bun test` | ci.yml test job | yes (after Q-048 resolution) | 22 file / **593 cases** — Bun native runner (post-AI-P1-12 INFRA-1B.5.h1-runbook-setup-hygiene landed: +24 tests in check_secrets_test.ts for pre-commit secret scanner — 6 filename guard + 2 redaction + 3 clean baseline + 3 env_file_staged + 8 per-vendor patterns + 1 regex isolation + 1 redaction safety; previous baseline 569 = AI-P1-1 INFRA-1B.4.h1-chunker-policy-gate) |
...
| `.github/workflows/ci.yml` | PR + push | **policy: required (DEC-020 Q-048 accepted) — branch protection admin task 미실시** (3-state 분리, 본 표 아래 "CI required check 등록 3-state" 참조) | bun install + typecheck + bun test (593 cases) + migrate dry-run |
...
- 코드 테스트 (`bun test`) 는 22 file / **593 케이스** — Bun native runner
  ...
  archive_policy gate + Codex P1 fail-closed, AI-P1-1) → **593**
  (INFRA-1B.5.h1-runbook-setup-hygiene landed 2026-05-15: +24 tests in
  check_secrets_test.ts for pre-commit secret scanner — `isStagedEnvFile`
  + `redactMatch` + `scanForSecrets` with 8 vendor patterns + regex
  `g`-flag state isolation, AI-P1-12).
