...
| unit tests | `bun test` | ci.yml test job | yes (after Q-048 resolution) | 25 file / **710 cases** — Bun native runner (post-Cycle-9 OPS-1B.h4-r2-audit-column-rationale-drift-axis landed: +6 tests in r2_invariant_scanner_test.ts — Axis 6 `r2_audit_column_rationale_drift` (column vs rationale dual-write contract violation surface) + 16 existing tests refactored with `columnSnapId`/`rationaleSnapId` fields; previous baseline 704 = Cycle 8 OPS-1B.h3-r2-orphan-axis-repairability). |
...
| `.github/workflows/ci.yml` | PR + push | **policy: required (DEC-020 Q-048 accepted) — branch protection admin task 미실시** (3-state 분리, 본 표 아래 "CI required check 등록 3-state" 참조) | bun install + typecheck + bun test (710 cases) + migrate dry-run |
...
- 코드 테스트 (`bun test`) 는 25 file / **710 케이스** — Bun native runner
