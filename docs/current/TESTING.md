...
> Last verified against code: this commit (2026-05-15) — AI-P1-2 / `INFRA-1B.1.h1-source-bootstrap-neo4j` landed (Neo4j Source node bootstrap + 3-way preflight alignment check + fail-fast `BootstrapPreflightError`; +15 tests in `tests/unit/neo4j_bootstrap_test.ts` = 521 → 536 tests total). Previous code baseline = 327f4b2 (2026-05-15, AI-P0-1 INFRA-1B.3.h1-policy-fix PR #41 — 515 → 521). Earlier baseline = 75706c4 (2026-05-14, INFRA-1B.3.x-audit PR #39 — 490 → 515).
...
| unit tests | `bun test` | ci.yml test job | yes (after Q-048 resolution) | 20 file / **536 cases** — Bun native runner (post-AI-P1-2 INFRA-1B.1.h1-source-bootstrap-neo4j landed: +15 tests in `tests/unit/neo4j_bootstrap_test.ts` for Neo4j Source bootstrap + 3-way preflight alignment; previous baseline 521 = AI-P0-1 INFRA-1B.3.h1-policy-fix PR #41 327f4b2) |
...
| `.github/workflows/ci.yml` | PR + push | **policy: required (DEC-020 Q-048 accepted) — branch protection admin task 미실시** (3-state 분리, 본 표 아래 "CI required check 등록 3-state" 참조) | bun install + typecheck + bun test (536 cases) + migrate dry-run |
...
- 코드 테스트 (`bun test`) 는 20 file / **536 케이스** — Bun native runner
  ...
  for cross-source archive_policy guard, AI-P0-1) → **536** (INFRA-1B.1.h1-
  source-bootstrap-neo4j landed 2026-05-15: +15 tests in neo4j_bootstrap_test.ts
  for Neo4j Source bootstrap + 3-way preflight alignment, AI-P1-2).
