...
| unit tests | `bun test` | ci.yml test job | yes (after Q-048 resolution) | 25 file / **704 cases** — Bun native runner (post-Cycle-8 OPS-1B.h3-r2-orphan-axis-repairability landed: +4 tests in r2_invariant_scanner_test.ts — Axis 4 cause-qualified rename (`r2_object_without_graph_key_set_failed`) + new Axis 4b (`r2_object_without_graph_key_policy_recheck_skipped` from `skipped_toctou`) + expectedR2Key in violation details for repair-CLI consumption + ScanCounts.skippedToctouAuditRows; previous baseline 700 = Cycle 7 INFRA-1B.3.h6 v9 schema hardening). |
...
| `.github/workflows/ci.yml` | PR + push | **policy: required (DEC-020 Q-048 accepted) — branch protection admin task 미실시** (3-state 분리, 본 표 아래 "CI required check 등록 3-state" 참조) | bun install + typecheck + bun test (704 cases) + migrate dry-run |
...
- 코드 테스트 (`bun test`) 는 25 file / **704 케이스** — Bun native runner
