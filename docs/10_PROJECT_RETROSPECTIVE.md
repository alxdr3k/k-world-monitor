...
- **NFR-003 5-hop trace 의무 vs 10-stage**: ADR-0025 가 ADR-0011 의 object model 만 supersede 하지만, NFR-003 (PRD `01_PRD.md:156`) 의 trace path Publication → ContentDraft → Thesis → Scenario → Claim → Snapshot → Source 가 이미 6 hop. EditorialIntent 추가 시 7 hop. **5-hop 표현이 ADR-0025 와 silent 충돌** → **Q-042 (사용자 결정 필요)**.
- **AC-044 (evidence_role minimum coverage, ADR-0027)** 가 `04_IMPLEMENTATION_PLAN.md:83` P0-M6 게이트 안에 들어가 있는데 **`06_ACCEPTANCE_TESTS.md` 에 정의 row 가 있는지 미검증** → **Q-043 (사용자 결정 필요)**.
- **PRD REQ-009/010 ADR ref 가 ADR-0006 인용** (이미 ADR-0023 으로 superseded). PRD §"Related Requirements" L309-321 에서 ADR-0023 / ADR-0024 / DEC-010 으로 reflow 필요 → **본 PR fix**.
- **HLD `02_HLD.md:226` "GDS plugin"** 이 v0 의무처럼 적혀 있는데 Q-024 resolved 결과는 "v0 APOC standard + Cypher 5.x core; v1+ GDS Community" → **본 PR fix**.
- **PRD `01_PRD.md:218` Q-004 status** 가 "open" 으로 적혀 있는데 `current-state.md:175` 는 resolved (INFRA-1A.2) → **본 PR fix**.

### 4) Operating risk surface (사용자 결정 필요)

- **NFR-008 "raw 0건 cloud 저장" audit 위치**: 코드에서 `policy_decisions` ledger row 가 R2 upload 시점에 어디서 INSERT 되는가? `src/storage/r2/policy.ts` 는 prefix check 만, `src/discovery/worker/snapshot-fingerprint.ts` 는 raw_cloud_policy check 만 — **audit ledger 명시적 INSERT 누락 위험** → **Q-044**.
- **Tier 0 호출 quota auditor**: DEC-010 cap "Tier 0 일일 5회" 가 코드에서 enforce 되는가? `src/ops/run-ledger.ts` 의 `startRun` 은 tier 0~3 record 만 하고 cap check 없음 — 호출자 책임 그대로 두면 silent drift 위험 → **Q-046**.
- **Secret management — Doppler vs `.env`**: API key 4종 (OpenAI / Anthropic / Google / R2) 의 운영 방법 결정 부재. `current-state.md:48-52` "v0 turn-key 진입 직전 Doppler 또는 환경 변수 등록 의무" 로 표현되어 있으나 ADR 부재 → **Q-047**.
- **CI branch protection**: 본 PR 이 `ci.yml` / `invariant-check.yml` 을 advisory (warning-level) 로 활성화. **required check 로 등록할지는 사용자 결정** → **Q-048**.
...
- 사용자 결정 불필요 fix 일괄 처리: thin docs SHA + 누락 파일 + AC TEST 위치 + IMPLEMENTATION_PLAN status + HLD/PRD supersede drift + closeDb/closeDriver finally hook + CI workflow rename (advisory) + `docs/_generated/` regen.
- 사용자 결정 필요 항목: `docs/questions/Q-042 ~ Q-048` 신규 등록.
