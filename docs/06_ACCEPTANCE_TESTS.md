...
| AC-020 | NFR-006 | Given Snapshot fingerprint row. When 원문이 변경/소실됨. Then (a) **모든 Snapshot 은 `canonical_text_hash` diff 로 의미 변경 감지** (Mozilla Readability + linkedom + normalize + sha256, ADR-0010 INV-0010-4 lock, Q-049 / DEC-021 resolution) — wrap diff (광고 / timestamp / tracker) false-positive 회피. `raw_body_hash` diff 단독은 stale trigger 가 *아니다* — `source_changed_minor` alert 로 분류 (v1+). canonical_text_hash NULL 인 Snapshot (P0 시점, INFRA-1B.9 도입 전) 은 fallback 으로 raw_body_hash diff 적용. (b) snapshot_diff trigger 발생 시 claim_status 가 source_changed / source_unavailable 로 자동 전이 (ADR-0010 INV-0010-1). (c) r2_key 가 NULL 이 아닌 permitted artifact (open-license dataset / 공식 API / 자체 산출물) 만 R2 round-trip 으로 회수 가능 — raw_body_hash + r2_key 무결성 verify. (d) raw third-party text 는 r2_key=NULL 이므로 R2 회수 대상 아님 (raw_cloud_policy=always_prohibited, NFR-008) | TEST-020 | defined |
