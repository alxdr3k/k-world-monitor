...
| AC-018 | NFR-003 | Given Publication. When 인용 trace 추출. Then **10-stage object model (ADR-0025) 안에서 5-hop 단축 trace path** = Publication → ContentDraft → Thesis → Scenario revision → Claim → Snapshot 으로 한 문장이 5단계 이내 역추적 가능 (DEC-020 Q-042 resolution, ADR-0025 INV-0025-5 intent surface). **EditorialIntent / Source / Document 는 선택적 skip 단계** — Thesis ↔ EditorialIntent 1:1 link 으로 reachable 보장 (metadata anchor 로 분류, trace 계산에서 제외 가능). **Source layer 의 Source / Document 는 Snapshot 의 metadata anchor — trace 종점은 Snapshot**. manual_claim_entry path 는 Snapshot 미포함 (4-hop 으로 추가 단축, ADR-0018). R2 bytes 는 permitted artifact 일 때만 trace 종착점이며 raw third-party text 는 미보관 (ADR-0012 INV-0012-3) | TEST-018 | defined |
| AC-019 | NFR-004 + DEC-010 | Given 일별 LLM 비용 상한 = **soft $5 / hard $7.5 / 주간 $25 / Tier 0 호출 일일 cap 5회 / backfill 별도 bucket** (DEC-010 lock). When run ledger 합산 (vendor + tier + cross_vendor_review_of + domain_override_reason 필드 포함, ADR-0023 INV-0023-7). Then (a) 누적 cost 가 soft 미만이면 정상 동작, (b) soft 도달 시 warn 알람 + 신규 queue throttling 시작, (c) hard 초과 시 queue backoff 강제 + 운영자 알람, (d) Tier 0 일일 5회 cap 초과 시 reject + 운영자 알람, (e) 주간 누적 $25 초과 시 다음 주 시작까지 backoff, (f) backfill bucket 은 daily cap 와 분리되지만 global emergency hard cap 은 공유. **Enforcement 위치 (planned, OPS-1A.2 슬라이스)**: `src/ops/quota-enforcement.ts` 일반화 quota module + QuotaKind enum (DEC-020 Q-046 resolution lock) — 본 모듈은 아직 미구현. 현재 OPS-1A.1 (run ledger + `getDailyCostUsd` / `getDailyCostBreakdown`) 까지 landed 이며, throttling worker + QuotaKind enforce 는 OPS-1A.2 진입 시 추가. AC-019 status `defined` = 기준 정의 완료, enforce 코드 미구현 (현재 evidence 없음). cross_vendor_review_coverage ≥ 0.95 KPI 도 OPS-1A.2 + EXTR-1A.6 (cross-vendor reviewer wiring) 시점 측정 | TEST-019 | defined |
| AC-020 | NFR-006 | **Phasing**: AC-020 은 P0 (fallback 만 검증) → P1 (canonical primary 활성) 2 단계로 분리. **P0-M2 gate scope = AC-020-P0 fallback only**: (a) 모든 Snapshot 은 `raw_body_hash` (current code 의 sha256 content_hash) diff 로 변경 감지 가능 + claim_status 가 source_changed / source_unavailable 로 자동 전이 (ADR-0010 INV-0010-1). canonical_text_hash 컬럼 미도입 (P0 시점, INFRA-1B.9 P1+ 까지 deferred) — diff trigger 는 raw_body_hash 단독. (b) r2_key 가 NULL 이 아닌 permitted artifact (open-license dataset / 공식 API / 자체 산출물) 만 R2 round-trip 으로 회수 가능 — raw_body_hash + r2_key 무결성 verify. (c) raw third-party text 는 r2_key=NULL 이므로 R2 회수 대상 아님 (raw_cloud_policy=always_prohibited, NFR-008). **P1+ gate scope = AC-020-P1 canonical primary** (INFRA-1B.9 / ADR-0010 INV-0010-4 / DEC-021 lock 후 활성): (d) `canonical_text_hash` diff (Mozilla Readability + linkedom + normalize + sha256) 가 의미 변경 trigger primary — wrap diff (광고 / timestamp / tracker) false-positive 회피. (e) `raw_body_hash` diff 단독은 P1+ 에서 `source_changed_minor` alert 로 분류 (NFR-009 thesis_polarity_distribution metric 과 별도). (f) canonical_text_hash NULL fallback 은 P1+ 에서도 raw_body_hash diff. P0-M2 gate accept 시점에 (d)/(e)/(f) 미구현 = 정상 (P1+ deferred), waived 또는 deferred 표기 불필요 — AC-020-P0 만 accept 대상 | TEST-020 | defined |
...
| AC-023 | REQ-018 | Given fetch / extract / cache / embed / cloud upload 단계. When `source_policy` 3 필드 (archive_policy / raw_cloud_policy / external_llm_policy) + **ADR-0017 INV-0017-4 의 8 위험 행동 (List A canonical, AI-P1-11 결정 lock 2026-05-17 — source-access 위험 도메인)** 트리거 검사 — 1. raw source 외부 LLM 전송 (`external_llm_policy != allowed`), 2. paywalled · proprietary source, 3. terms `no scraping` · `no AI` · `no archive` · `no redistribution`, 4. wire-service full text, 5. article·report 원문 quote · cache, 6. 도표·스크린샷 콘텐츠 추가, 7. raw source embedding · indexing, 8. raw source cloud upload. Then (a) **위험 행동은 어느 mode 에서도 inline_block 고정** (INV-0017-4 — AC-023 의 핵심 contract), (b) **비위험 행동은 stage-default mode** 적용 (INV-0017-3 — Discovery=inline_warn / Extract·Cache·Embed·Cloud upload=inline_block / 탐색·콘텐츠 제작=batch_report / Publication preflight=inline_block), (c) **모든 결정 (위험 / 비위험) 이 `policy_decisions` ledger 에 기록**된다 (INV-0017-5). 본 AC 의 evidence 구현 slice = **`INFRA-1B.5.h2-policy-gate-risk-triggers`** (AI-P1-11 D2a 결정, **landed via 본 PR — 53 tests pass**). retrospective 의 별개 8 위험 list (content-production safety — `promise_or_implication` / `legal_advice` / `medical_advice` / `financial_advice_specific` / `sensitive_attribute_inference` / `personal_identification` / `defamation_risk` / `policy_advocacy_or_partisan_endorsement`) 는 ADR-0017 범위 밖 (콘텐츠 발화 위험 도메인) — 별도 axis 로 P0-M6 publication 슬라이스에서 처리 (신규 ADR 후보) | TEST-023 | passing |
...
| TEST-018 | 10-stage 5-hop trace 단축 path (Publication → ContentDraft → Thesis → Scenario → Claim → Snapshot) — EditorialIntent / Source / Document 는 metadata anchor 로 선택적 skip (DEC-020 Q-042 lock) | `tests/pipeline/trace_test.ts` (planned) | AC-018 |
...
| TEST-020 | Snapshot fingerprint durability. **P0 (현재, P0-M2 gate scope)**: raw_body_hash (sha256 content_hash) diff 로 변경 감지 + claim_status source_changed / source_unavailable 자동 전이 + permitted artifact R2 round-trip (raw_body_hash + r2_key 무결성). **P1+ (INFRA-1B.9 진입 후)**: canonical_text_hash diff = 의미 변경 trigger primary (ADR-0010 INV-0010-4 / DEC-021 lock) + raw_body_hash diff 단독 = source_changed_minor alert + canonical NULL fallback = raw_body_hash. P0 시점 TEST scope 는 raw_body_hash + R2 round-trip 까지만; canonical assertion 은 INFRA-1B.9 슬라이스 안에서 TEST 본문 확장 | `tests/unit/snapshot_fingerprint_test.ts` ✓ tests pass (INFRA-1B.3, P0 raw_body_hash 부분) + `tests/unit/r2_policy_test.ts` ✓ 32 tests (INFRA-1A.3). P1+ canonical assertion 추가 의무는 INFRA-1B.9 | AC-020 |
...
| TEST-023 | policy gate mode-aware + 8 위험 행동 — ADR-0017 INV-0017-3 stage-default mode + INV-0017-4 mode-invariant inline_block + INV-0017-5 policy_decisions ledger persistence | `tests/policy/gate_test.ts` ✓ 70 tests pass (INFRA-1B.5.h2, 본 PR landed): 6 stageDefaultMode (+ stageDefaultMode invalid-stage throw runtime guard) + 38 detectRisks (8 trigger × fire/not-fire/mode-invariance + Codex PR #68 round 1 P1 fix: bare AP word-boundary + AFP/TASS word-boundary + 5 false-positive defense + Codex PR #68 round 2 P1/P2 fix: terms_violation extended to external LLM actions + detectExternalLlmRawTextUnauthorized sourceId=null fail-closed + Codex PR #68 round 3 P1/P2 fix: detectExternalLlmRawTextUnauthorized intended_action 확장 raw_text+excerpt + detectWireServiceFullText sourceName missing fail-closed when sourceId registered + GPT review post-PR-#68 옵션 D fix: Reuters Institute negative lookahead 3 cases + Reuters / Reuters News / Reuters Wire / Reuters World positive regression) + 10 evaluatePolicyGate (+ runtime fail-closed boundary guards invalid stage / invalid intendedAction) + 6 recordPolicyGateDecision (intended_action=NULL namespace + writer-boundary guard) + 9 TEST-023 E2E (8 trigger × ledger record × INV-0017-4 mode-invariance verified across 5 stages) | AC-023 |
