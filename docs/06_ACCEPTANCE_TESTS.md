...
| TEST-020 | Snapshot fingerprint durability — **canonical_text_hash diff primary** (의미 변경, ADR-0010 INV-0010-4 / DEC-021 lock) + **raw_body_hash diff = source_changed_minor alert** (v1+) + canonical NULL fallback = raw_body_hash + permitted artifact R2 round-trip (raw_body_hash + r2_key 무결성). 현재 코드는 raw byte sha256 (P0 baseline) — canonical_text_hash 도입 (INFRA-1B.9, P1+) 시 TEST 본문 확장 | `tests/unit/snapshot_fingerprint_test.ts` ✓ tests pass (INFRA-1B.3, raw_body_hash 부분) + `tests/unit/r2_policy_test.ts` ✓ 32 tests (INFRA-1A.3). canonical_text_hash test assertions 는 INFRA-1B.9 슬라이스 안에서 추가 | AC-020 |
