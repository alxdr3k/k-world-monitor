...
- current milestone: `P0-M3` (Extraction & Review — Cycle 38 EXTR-1A.* phase 진입, EXTR-1A.0 landed via PR #97 49d4a79; Cycle 39 EXTR-1A.1 extractor router landed via PR #98 5d037d3; Cycle 40 EXTR-1A.2a article extractor skeleton landed via PR #99 dd49e63; Cycle 41 EXTR-1A.2b real OpenAI Tier 2 + run_ledger landed via PR #100 c9034ba; Cycle 42 follow-up #1 pricing ratification landed via PR #101 1c49717; Cycle 42 follow-up #2 EXTR-1A.2d article extraction strict JSON schema + DEC-025 ratification + codex round 1 P2 ordering fix landed via PR #102 8626dd1; Cycle 43 EXTR-1A.2c.1 Anthropic Messages REST client skeleton landing via 본 PR). **P0-M2 (Source Registry & Collection Queue) 는 모든 own slice landed 상태에서 게이트 검증 대기 — gate accept evidence (SPIKE-001 + AC-022/023/024) 확정 후 closed.** **P0-M2-hardening engineering sequence: 17 code hardening PR landed (16 historical Cycle 7/8/9/10 post-#58 + post-#65 GPT review followup + `INFRA-1B.5.h2-policy-gate-risk-triggers` AC-023 / TEST-023 evidence engineering — landed via PR #68 squash merge 2026-05-18 408cf2e, AI-P1-11 D1a/D2a 결정 lock, 798 tests pass)** + **Cycle 12 NOT-gate-blocking refactor: `INFRA-1A.x-shared-snapshot-id-constants` (shared snap_id + R2 prefix constants module 신설, 4 site magic-string duplication 종결, drift surface single source 확정 — PR #66 Cycle 10 Finding 6 anchor consolidation; PR #69 squash merged 2026-05-18 c996775, 798 → 816 tests)** + **Cycle 13 follow-up: `formatSnapIdRationalePrefix` writer-side helper (Cycle 12 Opus review P3 informational defer 처리, symmetric drift surface 종결; PR #71 squash merged 2026-05-18 742af9b, 816 → 820 tests)** + **Cycle 14 NOT-gate-blocking validator extension: `INFRA-1A.9-validator-extension` (invariant validator `cross_ref_code` 검증 추가 + INV-0012-3 backfill — DEC-020 Q-045 / TRACE-041 resolution; 본 PR pending, 820 → 833 tests; INV-0028-* / INV-0023-3 / INV-0017 backfill 은 다음 cycles)** + **Cycle 34 NOT-gate-blocking config validator: `INFRA-1A.9.h4-llm-routing-config-validator` (operator D3 2026-05-18 — `scripts/check-llm-routing-config.ts` 신규 pure config validator for ADR-0023 INV-0023-2 (Tier 0 vendor + effort lock) + INV-0023-3 (capability canonical, NOT price-axis 키 또는 값) + INV-0023-5 (Google scope = Tier 3 only, all-tier scan + role allowlist). LLM 호출 0, runtime wiring 0 — pure validator. data/llm_routing.yaml `tier_3.capability` 가 `low_cost_high_volume_and_search_grounding` 에서 `high_volume_throughput_and_search_grounding` 으로 갱신 (codex review P2 round 1 — capability value 가 price-axis token 포함 금지 contract). 970 → 1037 tests post-codex-3-rounds (37 initial + 25 round-1 + 5 round-2 = 67 신규). PR #93 merged 2026-05-19 80182d5)** + **Cycle 35 NOT-gate-blocking policy validator: `INFRA-1A.9.h5-vault-jsonl-policy-validator` (operator D4 2026-05-18 + 1 codex round hardening — `scripts/check-vault-jsonl-policy.ts` 신규 pure policy validator for ADR-0012 INV-0012-5 (Markdown vault content kinds — Document hub / Dossier / Scenario / Thesis / ContentDraft / Publication / promoted_claim / **editorialintent** [ADR-0025 added per codex P2] 만 permit; codex P2 — MDX 확장자 walk 추가 `VAULT_FILE_EXTENSIONS = [".md", ".mdx"]`; codex P2 — `assertPromotedClaimsAreCited` 신규 enforce — promoted_claim 의 claim_id 가 scenario frontmatter `cited_claims[]` 또는 scenario body whole-word 에 존재해야 함, INV-0012-5 "scenario에 인용된" clause closure) + INV-0012-6 (JSONL not canonical store — ALLOWED_JSONL_PATH_PREFIXES allowlist 외 *.jsonl 거부). vault 디렉터리 부재로 INV-0012-5 는 현재 vacuously true (future-guard); INV-0012-6 는 live `.dev-cycle/*.jsonl` 2개에 대해 allowlist 통과. ADR-0012 INV backfill 7/7 완료 (INV-0012-5 는 cross_ref 2 targets — assertVaultContentKinds + assertPromotedClaimsAreCited). 1037 → 1086 tests post-codex-round-1 (49 신규: 4 live + 12 vault content + 9 assertPromotedClaimsAreCited + 12 JSONL allowlist + 12 filesystem round-trip incl. MDX scan + orphan throw + happy-path). PR #94 merged 2026-05-19 2f2430f)** + **Cycle 36 NOT-gate-blocking glossary backfill: `INFRA-1A.10-glossary-backfill` (operator D5 2026-05-18 defer-eligible filler — landed via PR #95 doc-only): 3 신규 glossary entry — `retention_protected_kinds` (DEC-007 enum lock, GC batch reject list) + `data_science_module` (ADR-0024 + DEC-010 + INV-0023-6, mandatory_for_dataset_over_1000_rows_or_50kb) + `cross_vendor_reviewer` (DEC-010 + INV-0023-4 enforcement_scope, 3 단계 lock). validator WARN 5 → 2 (3 glossary missing 종결; 잔존 2건 README frontmatter id 누락은 separate hygiene). pure docs-only — code baseline 2f2430f 유지 (AGENTS.md doc-only rule). tests 1086 pass / 0 fail no delta. AI-P2-5 anchor close)** + **Cycle 37 NOT-gate-blocking future-implementation inventory: `INFRA-1A.11-future-implementation-inventory` (operator D6 2026-05-18 — landed via PR #96 doc-only): `docs/context/future-implementation-inventory.md` 신설 — 19 design-stage ADRs (0003 / 0004 / 0005 / 0007 / 0013 / 0014 / 0015 / 0018 / 0019 / 0020 / 0021 / 0022 / 0024 / 0025 / 0026 / 0027 / 0029 / 0030 / 0031, 총 117 invariants) 를 ADR-level entries 로 분류: 3 superseded + 2 partially-implemented + 14 design-only. 각 entry 에 activation_phase + current_status + enforcement_status + reason 명시. validator policy 명시 — future-implementation invariant 의 `cross_ref_code` 부재는 violation 이 아님. 미래 production wiring slice land 시 해당 PR 이 cross_ref_code 추가 + inventory entry 갱신 의무. pure docs-only. tests 1086 pass / 0 fail no delta. D6 close)** + **Cycle 38 P0-M3 EXTR-1A.* phase 진입 — first slice `EXTR-1A.0-prompt-injection-defense` (operator "extra 진입" 2026-05-19 — D7 production-caller-0 reactivation 의 phase 전환): ADR-0029 LLM Prompt Injection 방어 기반 3 modules + 123 tests (6 codex hardening rounds). `src/extraction/sanitize/html-to-text.ts` (INV-0029-5 — DANGEROUS_TAGS 6종 완전 제거 [round 6 P2 greedy closed regex — nested same-tag body-leak 종결] + entity decode + comment/CDATA-unwrap/DOCTYPE/PI strip [round 6 P2 — post-decode 재실행으로 entity-encoded `&lt;!-- ... --&gt;` / `&lt;!DOCTYPE&gt;` bypass 종결] + quote-aware tag stripper + MALFORMED_TAG_FALLBACK + iterative entity decode until stable [MAX_DECODE_ITER=100, fail-closed when not stabilized — round 5 P2 6+/10-level nested ampersand bypass 종결] + semicolonless legacy entities `&lt`/`&gt`/`&amp`/`&quot` [HTML5 §13.5 legacy — round 5 P2] + semicolonless numeric entities `&#60script` / `&#x3Cscript` [HTML5 §13.2.5.74 legacy — round 6 P2]), `src/extraction/prompt/untrusted-wrapper.ts` (INV-0029-1 sentinel + INV-0029-3 per-tier 토큰 상한 TIER_TOKEN_CAPS + CHARS_PER_TOKEN_HEURISTIC=1 universally-safe + escapeSentinelLiterals 가 tag-form whitespace/case 변형 [`</untrusted >` / `< /untrusted>` / `</UNTRUSTED >`] 까지 escape — round 5 P2 tag-form sentinel bypass 종결), `src/extraction/policy/llm-policy-gate.ts` (INV-0029-4 — checkLlmPolicy 가 source_material_policy.external_llm_policy 조회 후 prohibited/manual_review_required/unregistered fail-closed throw). ADR-0029 INV-0029-1/3/4/5 cross_ref_code 4 entries 추가 (INV-0029-2 코드 패턴 금지 invariant 별도 anchor 후속). future-implementation-inventory 의 ADR-0029 entry: design-only → partially-implemented. 1086 → 1209 tests on main. PR #97 merged 2026-05-19 49d4a79. EXTR-1A.1 (extractor router) 개시 가능 상태)** + **Cycle 39 EXTR-1A.* phase 두번째 slice `EXTR-1A.1-extractor-router` (operator 표준 직정 "계속 진행 이슈 없으면 멈추지 마" — D7 sequence 의 EXTR-1A.0 unblock 직후 진입): article/dataset/report 분기 router + interface contract scaffolding (AC-009 + AC-021 / NFR-007). 4 modules in `src/extraction/router/`: types.ts (SOURCE_TYPE = [article, dataset, report] + SourceType + isSourceType type guard + ExtractorInput + ExtractorOutput + Extractor interface), registry.ts (ExtractorRegistry Map-backed + 3 typed errors — ExtractorNotRegisteredError fail-closed + ExtractorAlreadyRegisteredError dup-prevent + InvalidSourceTypeError off-canonical-defend), router.ts (routeAndExtract pure dispatch + envelope validation + post-dispatch envelope-consistency fail-closed [extractor 가 wrong sourceType/sourceId 반환 시 throw]), index.ts (public re-export). **Thin router** — content sanitization (ADR-0029 INV-0029-5) / token cap (INV-0029-3) / LLM policy gate (INV-0029-4) / LLM 호출 / Data Science Module 호출은 모두 concrete extractor 의무 (EXTR-1A.2 onward). 25 신규 tests (tests/unit/extractor_router_test.ts) = SOURCE_TYPE enum + ExtractorRegistry + 3 typed errors + TEST-009 dispatch + envelope validation + extractor-throw propagation + input-mutation snapshot defense + envelope-consistency fail-closed + TEST-021 dry-run extensibility 모두 cover. 1209 → 1234 tests on main. PR #98 merged 2026-05-19 5d037d3. EXTR-1A.2 (article extractor) 개시 unblock)** + **Cycle 40 EXTR-1A.* phase 세번째 slice `EXTR-1A.2a-article-extractor-skeleton` (operator delegated cut "그럼 몇 개의 슬라이스로 나눠서 작업해" 2026-05-19 — EXTR-1A.2 parent 가 3 sub-slices 로 분해: 2a article extractor with mock LLM + INV-0029-* defensive pipeline, 2b OpenAI Tier 2 default + run_ledger, 2c Korean long-context Sonnet override + domain_override_reason): 2 modules in `src/extraction/llm|article/`: client.ts (`LlmClient` interface + `LlmInvokeParams`/`LlmInvokeResult` + `LlmTier` type — vendor-agnostic abstraction), article-extractor.ts (`ArticleExtractor implements Extractor` + `ARTICLE_EXTRACTION_SYSTEM_PROMPT` default INV-0029-1 caller-warning + 5-step pipeline: `checkLlmPolicy(sourceId, db)` fail-closed → `htmlToText(rawContent)` INV-0029-5 → `wrapUntrustedForTier(plain, tier)` INV-0029-1 + INV-0029-3 → `llmClient.invoke({systemPrompt, userPrompt: wrapped, tier})` → envelope build). Concrete LLM client (real OpenAI/Anthropic SDK wiring) + run_ledger 통합은 EXTR-1A.2b 으로 defer. 24 신규 tests covering INV-0029-1/3/4/5 defensive pipeline + registry integration + multi-vector adversarial defense-in-depth (script + entity payload + nested + sentinel injection + sanitization-strips-fake-close + payload bounded inside wrapper) + round 2 P2 caller systemPrompt 가 mandatory INV-0029-1 warning 을 drop 못함 (warning prepend + override append). 1234 → 1258 tests on main (post-codex-round-2: +1 mandatory caller-warning test). PR #(본 PR) pending. EXTR-1A.2b (real OpenAI Tier 2 default client + run_ledger 통합) 개시 unblock)** + **Cycle 41 EXTR-1A.* phase 네번째 slice `EXTR-1A.2b-openai-client` (operator option A "OpenAI SDK 실제 wiring + run_ledger 한 PR" 2026-05-19): Real OpenAI Chat Completions REST client (fetch-based, no `openai` npm dep) + LlmClient interface extension (readonly `vendor`/`tier`/`model` + LlmInvokeResult.totalCostUsd?) + ArticleExtractor OPS-1A.1 run_ledger integration (startRun BEFORE invoke + completeRun on success + failRun on throw; mock-vendor LlmClient skips ledger). OpenAIClient constructor `{apiKey?, tier=2, model?, fetch?, baseUrl?}` with apiKey fail-fast + OPENAI_API_KEY env fallback. Per-tier default model OPENAI_TIER_DEFAULT_MODEL (Tier 2 = `gpt-5-mini` per ADR-0023 §statement). computeTotalCostUsd from OPENAI_PRICING_USD_PER_1M_TOKENS placeholder pricing (TODO ADR-0023 ratification, unknown model fail-closed). OpenAIApiError carries status + responseBody. ArticleExtractor refactor — drops `db` + `tier` injection (now reads from llmClient.tier + getDb() default), adds `runStage` + `domainOverrideReason` deps for EXTR-1A.2c Anthropic Sonnet override forward. Test fixture migration ArticleExtractor: `db` injection → `process.env.SQLITE_PATH=:memory:` + `closeDb()` + bootstrap (matches run_ledger_test pattern). 28 신규 tests (23 openai_client + 5 article_extractor OPS-1A.1 integration), 1258 → 1375 tests post-Cycle-42-ratification on main. PR #100 merged 2026-05-19 c9034ba. Cycle 42 follow-up #1 pricing ratification PR #101 merged 2026-05-19 1c49717. EXTR-1A.2c (Korean long-context Sonnet override) 개시 unblock)** + **Cycle 42 follow-up #2 EXTR-1A.* phase 다섯번째 slice `EXTR-1A.2d-article-extraction-strict-json-schema` (operator post-merge direction 2026-05-19 "minimal 로 하고 나머지 extended 는 별도 슬라이스로 지금 저장" + "분리 (schema 도입만 + quote validation 은 후속 cycle)" — DEC-010 §8 strict schema invariant + AC-010 (c) wiring): ArticleExtractor 에 `ARTICLE_EXTRACTION_JSON_SCHEMA` (OpenAI `response_format: { type: "json_schema", strict: true }` shape with required `[title, summary, key_claims]` + `additionalProperties: false` 양 levels) + `ExtractedArticle` interface (camelCase) + `ArticleExtractionSchemaError` typed + `parseExtractedArticle()` minimum form validator + extract() 가 schema 를 invoke 에 responseFormat 으로 전달 + LLM raw text 를 invoke 후 JSON.parse + 형식 validation 실패 시 failRun(runId, {tokens+cost}) 로 billable usage payload 보존 후 rethrow (round-4 F13 invariant 그대로). Envelope `result` 에 `parsed: ExtractedArticle` 추가 (raw `text` 도 audit / regression diff 용 보존). System prompt 에 verbatim evidence_quote 지시 추가 (faithfulness anchor — quote substring validation 은 2f). +13 tests (39 → 52 in article_extractor_test) — schema shape const + responseFormat passthrough + parsed surface + raw-text retention + non-JSON throw + non-object root throw + missing title/summary/key_claims throws + key_claims-not-array throw + missing evidence_quote throw + empty key_claims accepted + real-vendor malformed JSON → failRun with billable payload preserved + system prompt verbatim assert. **Same PR codex round 1 P2 fix + DEC-025 ratification**: missing-cost guard moved BEFORE schema parse (dual-vendor-bug 시 malformed JSON + totalCostUsd undefined 동시 발생할 때 AC-019 missing-cost error 가 우선 surface 되도록), +1 test (52 → 53 in article_extractor_test). docs/decisions/DEC-025.md 신설 (article extraction strict JSON schema v1 minimum field set lock + run_ledger ordering invariant + extended fields/quote substring validation 명시 deferred). 1376 → 1390 tests on main. PR #102 merged 2026-05-19 8626dd1. **Cycle 43 EXTR-1A.* phase 여섯번째 slice `EXTR-1A.2c.1-anthropic-client-skeleton` (operator option (4) `/dev-cycle --loop` 자동 진행 2026-05-19 — parent EXTR-1A.2c 가 PR #99/100 precedent 에 따라 2c.1 skeleton + 2c.2 vendor swap 으로 cut): `src/extraction/llm/anthropic-client.ts` (`AnthropicClient implements LlmClient` — fetch-based Anthropic Messages API, no `@anthropic-ai/sdk` dep) + 4 typed errors (`AnthropicApiError` / `AnthropicRequestTimeoutError` / `AnthropicPricingUnknownError` / `AnthropicIncompleteCompletionError`) + ANTHROPIC_TIER_DEFAULT_MODEL (Tier 0=Opus 4.7 placeholder / Tier 1=Sonnet 4.6 / Tier 2=Sonnet 4.6 Korean override default / Tier 3=Haiku 4.5) + ANTHROPIC_PRICING_USD_PER_1M_TOKENS placeholder (ratification follow-up parallel to PR #101) + computeAnthropicTotalCostUsd + resolveAnthropicPricingKey (longest-prefix dated-snapshot lookup) + DEC-010 §8 strict-schema vendor 매핑 (json_schema responseFormat → Anthropic forced-tool contract `tools[].input_schema` + `tool_choice: {type:"tool", name}`, tool_use input JSON-stringify 으로 OpenAI 와 동일 wire form). PR #100 OpenAIClient 모든 hardening 패턴 적용: pre-API pricing validation (unknown SKU 가 ledger row 열기 전 throw), Tier 0 explicit `model` 의무, timeoutMs positive-finite check, AbortController 가 body 읽기까지 cover, strict usage validation (input/output non-negative integers, cached ≤ input), incomplete-completion reasons (`max_tokens` / `missing_tool_use` / `malformed_tool_input` / `missing_content` / `missing_usage` / `usage_inconsistent`) 모두 LlmIncompleteResultError.usage payload 보존 (AC-019 invariant). +43 신규 tests in `tests/unit/anthropic_client_test.ts` (constructor 9 / request shape 6 / response parsing 5 / error paths 2 / incomplete rejection 9 / pricing math 6 / dated-snapshot lookup 4 / LlmClient interface compliance 2). **Same PR codex round 1 P1/P2 fix (4 finding)**: Anthropic 의 `input_tokens` / `cache_read_input_tokens` / `cache_creation_input_tokens` 가 **별도 non-overlapping bucket** 인 점을 잘못 OpenAI subset-style 로 차용. (1) computeAnthropicTotalCostUsd 가 `nonCached = input - cached` 빼지 않고 각각 별도 rate 으로 sum — cache-heavy 트래픽에서 cost undercount 종결, (2) `cached > input` usage_inconsistent veto 제거 — cache-hit 가 standard input 초과하는 정상 케이스 false positive 종결, (3) cache_creation_input_tokens 파싱 + 125% standard input rate 으로 cost 반영 — cache write 누락 종결, (4) tool_use.input 형 검사를 object-only → undefined-only 로 완화 — JSON Schema spec 의 non-object root (array / null / scalar) 정상 처리. ANTHROPIC_PRICING_USD_PER_1M_TOKENS 의 모든 SKU 에 cacheCreationInput rate 추가 (Opus/Sonnet/Haiku 각각). +5 net tests (43 → 48 in anthropic_client_test). 1390 → 1438 tests on main. Korean long-context detection + ArticleExtractor vendor swap + domain_override_reason field write = EXTR-1A.2c.2 신규 sibling slice (planned). PR #(본 PR) pending merge SHA)**. 2 신규 planned sibling slices 동시 등록: EXTR-1A.2e (extended fields publication_date + author + topics) + EXTR-1A.2f (quote substring validation + faithfulness_rate KPI + run_ledger schema 확장). PR #(본 PR) pending merge SHA)**. AC-031 source-side enum reject 는 `INFRA-1B.1.h4-category-enum-validation` planned follow-up anchor 로 등록되어 있으나 **blocked/sequence-dependent on `INFRA-1B.1.h2-source-profile` (categories.yaml owner) — P0-M2 gate accept evidence 에서 제외** (운영자가 명시적으로 AC-031 을 gate evidence 에 promote 하는 경우에만 변경). 세부 landed (`INFRA-1B.3.x-audit` PR #39 landed [R2 upload audit ledger, AC-032 / NFR-008] + `INFRA-1B.3.h1-policy-fix` PR #41 landed [R2 cross-source archive_policy guard, AI-P0-1 legal-safety P0] + `INFRA-1B.1.h1-source-bootstrap-neo4j` PR #44 landed [Neo4j Source node bootstrap + 3-way preflight, AI-P1-2] + `INFRA-1B.3.h2-queue-cli` PR #45 landed [`bun run discovery:process-queue` CLI + new-path `source_not_found_in_graph` TypedQueueError unification + `parseArgs` allowlist fail-fast, AI-P1-3] + `INFRA-1B.4.h1-chunker-policy-gate` PR #47 landed [chunker archive_policy gate + empty-text preserve + `ChunkRejected` 5 reason enum + fail-closed inverted allowlist, AI-P1-1 / Q-053 D2 / DEC-024 D2] + `INFRA-1B.5.h1-runbook-setup-hygiene` PR #48 landed [RUNBOOK fresh-worktree setup + pre-commit secret scanner — 9 vendor pattern + `execFileSync` argv form + 256MB maxBuffer + `--no-renames` `--diff-filter=ACMT`, AI-P1-12 operator safety baseline] + `INFRA-1B.3.h3-audit-hardening` PR #49 landed [v8 audit hardening — `policy_decisions.upload_attempt_id` + 3 BEFORE INSERT triggers with explicit whitespace charset TRIM, AI-P1-7 BEFORE/AFTER audit pair correlation key] + `OPS-1B.h1-runtime-invariant-scanner` PR #50 landed [R2 invariant scanner — Snapshot.r2_key ↔ policy_decisions uploaded audit ↔ source_material_policy 3-way reconciliation with 3 violation axes, AI-P1-6 INV-0012-3 audit-by-absence cross-check] + `INFRA-1B.3.h4-dedup-r2-backed-link-policy-fix` PR #53 landed [snapshot-fingerprint line 490 dedup-link guard allowlist 확장 — PR #41 sibling call site hole 종결, AI-P0-2 legal-safety P0 post-PR #51 review reopen] + `OPS-1B.h2-r2-invariant-scanner-orphan-axis` PR #54 landed [scanner SQL `decision IN ('uploaded', 'set_r2_key_failed_neo4j')` + Axis 4 `r2_object_without_graph_key` + Axis 5 `malformed_r2_upload_audit_row`, AI-P1-13 P1 gate-blocker — PR #50 set_r2_key_failed_neo4j orphan + defensive coding regression 종결] + `INFRA-1B.1.h3-seed-sources-argv-allowlist` PR #56 landed [scripts/seed-sources.ts `parseArgs` allowlist + `UnknownArgumentError` + `import.meta.main` entry guard, AI-P1-14 — `--dryrun --neo4j` typo silent-write risk 종결] + `INFRA-1B.3.h5-policy-decisions-snap-id-column-v9` PR #57 landed [v9 migration: snap_id TEXT column + partial INDEX + recordR2UploadDecision dual-write + scanner column-preferred + `validSnapIdOrNull` shape guard, AI-P1-15 + Codex round 1 P1/P2 fix — scanner 의 rationale regex 의존 구조적 해소] + `INFRA-1B.3.h6-policy-decisions-snap-id-schema-hardening` PR #62 landed [writer-boundary `assertValidSnapId` fail-fast at recordR2UploadDecision + v8→v9 migration integration test + DATA_MODEL.md v9 sync, Cycle 7 — post-#58 GPT review P1 finding 3건 (writer boundary / migration path / DATA_MODEL) 종결] + `OPS-1B.h3-r2-orphan-axis-repairability` PR #63 landed [scanner SQL `decision IN ('uploaded', 'set_r2_key_failed_neo4j', 'skipped_toctou')` 확장 + Axis 4 cause-qualified rename `r2_object_without_graph_key_set_failed` + 신규 Axis 4b `r2_object_without_graph_key_policy_recheck_skipped` + 양 orphan axis details 에 `expectedR2Key` 출력 + ScanCounts.skippedToctouAuditRows + RUNTIME.md scanner section 신설 (Codex P1) + TESTING SHA placeholder 교체 (Codex P2), Cycle 8 — post-#58 GPT review Issue 3 종결 + 정책 갱신 (AGENTS.md "engineering slice PR sync 의무")] + `OPS-1B.h4-r2-audit-column-rationale-drift-axis` PR #64 landed [scanner Axis 6 `r2_audit_column_rationale_drift` 신설 — column ↔ rationale dual-write contract violation surface (recordR2UploadDecision atomic dual-write 의 divergence 를 explicit violation 으로 노출, scanner column-preferred resolution 이 silently mask 하던 v8- legacy consumer-visible anomaly 종결); R2UploadOutcomeAuditRow shape 에 raw `columnSnapId` + `rationaleSnapId` field 노출; Cycle 9 — post-#58 GPT review Issue 6 (마지막 P2 finding) 종결] + `INFRA-1B.3.h7-gate-evidence-hardening` (PR #66 merged 2026-05-17, fdb847a — Cycle 10) landed [parseSnapIdFromRationale `(?=;|$)` delimiter strictness lookahead + invalid trailing char regression tests (7) + snapshot-fingerprint skipped_toctou rationale mergeMatchedExisting branch + regression test (2) + policy-decisions / scanner docstring 의 `snap_<ULID>` 표현을 실 정규식 매칭 표현으로 정정 + thin-doc drift sweep (TESTING / RUNTIME / DATA_MODEL header SHA backfill + RUNTIME 6-axis → 7-axis 운영자 CLI flow 정정), Cycle 10 — post-PR #65 GPT review Findings 1~5 종결 (Finding 6 `INFRA-1A.x-shared-snapshot-id-constants` 는 planned anchor 로 register)]). 세부 planned (gate evidence 제외, follow-up anchor): `INFRA-1B.1.h4-category-enum-validation` (AC-031 source-side enum reject, blocked-on-h2, planned anchor registered via PR #59 — operator 가 h2/h4 sequencing 결정 시 promote 가능). P0-M1 게이트도 별도 통과 필요 (SPIKE-001 미실시 — AC-032 audit ledger 코드 enforcement 는 INFRA-1B.3.x-audit 로 landed, Q-044 는 DEC-020 으로 resolved, TRACE-040 anchor → landed).
- active tracks: `EXTR` (primary — Cycle 38 EXTR-1A.* phase 진입, EXTR-1A.0 prompt injection 방어 기반 landed), `INFRA` (cross-cutting — INFRA-1A.* hardening backfill 계속, INFRA-1B 게이트 검증 대기), `OPS` (cross-cutting — OPS-1A.1 run ledger landed)
- active phase: `EXTR-1A` (Cycle 41 — EXTR-1A.0 + EXTR-1A.1 + EXTR-1A.2a merged, EXTR-1A.2b real OpenAI Tier 2 + run_ledger 본 PR pending; EXTR-1A.2c Sonnet override 남음); `INFRA-1B` (게이트 검증 단계); `OPS-1A` (게이트 검증 단계 + 본 PR 으로 actual run_ledger consumer wiring 첫 site)
- active slice: **EXTR-1A.2b-openai-client** (Cycle 41 본 PR — Real OpenAI Chat Completions REST client + LlmClient interface extension + OPS-1A.1 run_ledger 통합 + ArticleExtractor refactor, AC-007 + AC-010 + AC-019; 3 modules in `src/extraction/llm|article/` + 23 신규 + 5 update OPS-1A.1 integration tests; Anthropic Sonnet override + Korean long-context detection 은 EXTR-1A.2c 후속 slice 영역 — operator option A "OpenAI SDK 실제 wiring + run_ledger 한 PR" 2026-05-19). **Prior cycles**: EXTR-1A.2a-article-extractor-skeleton (Cycle 40 PR #99 merged dd49e63, AC-009 + AC-010 skeleton + INV-0029-* mock pipeline, 2 codex rounds, 1234 → 1258 tests), EXTR-1A.1-extractor-router (Cycle 39 PR #98 merged 5d037d3, AC-009 + AC-021 / NFR-007 router scaffolding, 4 modules + 25 tests, 1209 → 1234 tests, codex pass first review), EXTR-1A.0-prompt-injection-defense (Cycle 38 PR #97 merged 49d4a79, ADR-0029 INV-0029-1/3/4/5 enforcement, 6 codex hardening rounds, 1086 → 1209 tests). **Prior landed slices**: M2 own slices INFRA-1B.1 ~ INFRA-1B.5 (P0-M2 gate accept evidence 대상) + cross-milestone early landed P0-M3 slices INFRA-1B.6 (feedback CLI, PR #22 7f4e980) + OPS-1A.1 (run ledger, PR #23 23de14c). P0-M2 게이트 검증 미실시 evidence: SPIKE-001 (NFR-001 1만 graph object < 1s p95), AC-022/023/024 evidence 확정. 2026-05-13 comprehensive review 가 식별한 doc-code drift 일괄 backfill 됨. **P0-M2-hardening engineering sequence + INFRA-1A.9 follow-up (Cycle 14-37) 모두 landed** (operator D1-D6 sequence 완료, D7 phase 진입 시작). Landed M2 own PR 목록: #15 (INFRA-1B.1 source registry seed + 1B.1.x hotfix, c51b2ce), #16 (INFRA-1B.2a safe-fetch, ed09aa5), #17 (INFRA-1B.2b scheduler, 0eec962), #18 (INFRA-1B.2 discovery worker, 896ddf2), #19 (INFRA-1B.3 snapshot fingerprint, 4dfa94f), #20 (INFRA-1B.4 chunker, 06c49d7), #21 (INFRA-1B.5 access-intervention, c3b19c4), #22 / #23 (early-land P0-M3 slices).
