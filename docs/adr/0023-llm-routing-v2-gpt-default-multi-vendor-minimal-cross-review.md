...
invariants:
  - id: INV-0023-1
    ...
    status: active
  - id: INV-0023-2
    statement: Tier 0 (frontier reasoning, judgment / validation) = **GPT-5.5 Pro extended thinking** + **Claude Opus 4.7 xhigh effort**. Google (Gemini) 사용 절대 금지. 두 모델은 성능/특성 기준으로 동 tier (Opus 4.7 = source-grounded reasoning + agentic long-task 우위, GPT-5.5 Pro xthink = 일반 frontier reasoning 우위). 가격 단독 매핑 금지.
    status: active
  - id: INV-0023-3
    statement: Tier 매핑 기준 = **성능 / 분야 우위 / 특성** (가격 단독 매핑 금지, INV-0023-5 강화). vendor 별 동 tier 모델은 quality benchmark + domain advantage 로 매핑되며 cost 는 보조 지표.
    status: active
  - id: INV-0023-4
...
### 1. Tier × Vendor 매핑 (성능 / 분야 / 특성 기준)

| Tier | 용도 | OpenAI (default) | Anthropic | Google | 매핑 근거 (성능/분야) |
|---|---|---|---|---|---|
| **Tier 0 (frontier reasoning)** | Scenario validate (3') / high-stakes Thesis / cross-vendor preflight review | **GPT-5.5 Pro extended thinking** (default) | **Claude Opus 4.7 xhigh effort** (cross-vendor review only) | **사용 금지** (INV-0023-2) | Opus 4.7 = source-grounded reasoning + agentic long-task SOTA / GPT-5.5 Pro xthink = 일반 frontier reasoning SOTA. 두 모델 동 tier — 가격 단독 매핑 X. |
| **Tier 1 (high reasoning, primary)** | Scenario composer / Thesis composer (default) / Dossier 합성 (reasoning-heavy) | **GPT-5.5 Pro standard** (default) | Claude Sonnet 4.6 high effort (long-context dossier ≥ 100K tokens 시 override) | **사용 금지** (메인/리뷰 X) | Sonnet 4.6 = 200K context window 우위 (long-form 분석 / dossier 합성) / GPT-5.5 Pro = 일반 reasoning 우위. context length 가 도메인 override trigger. |
| **Tier 2 (mid, structured extraction)** | Article extract (default) / Report extract / Dossier 합성 (기본) / EvidencePack 4-section | **GPT-5 mini** (default) | Claude Sonnet 4.6 standard (Korean-heavy 문서 + 1만 tokens 이상 context 시 override) | **사용 금지** (메인/리뷰 X) | GPT-5 mini = structured tool-use + JSON validity 우위 / Sonnet 4.6 standard = 한국어 long-context 우위 (Korean source 우위 입증 시 override). |
| **Tier 3 (low, high-volume)** | Cite check LLM judge / EvidencePack 단순 변환 / 검색 후처리 / search grounding fallback | **GPT-5 nano** (default) | Claude Haiku 4.5 (cross-vendor preflight review only) | **Gemini 2.5 Flash** (Google Search grounding 작업 + 동일 tier 비용효율 우위 시만, INV-0023-5) | Haiku 4.5 = cite-check judge faithfulness 우위 / Gemini Flash = Search grounding + ground-truth citation 우위 (다만 메인/리뷰 X) / GPT-5 nano = structured high-volume default. |
