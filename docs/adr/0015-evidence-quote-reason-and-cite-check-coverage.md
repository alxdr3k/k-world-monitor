...
id: adr-0015
type: adr
title: Evidence (nullable quote + quote_reason + storage_level) and cite check 5+1 block + warning (v1+) (supersedes ADR-0008)
status: accepted
...
invariants:
  ...
  - id: INV-0015-4
    ...
    status: active
  - id: INV-0015-5
    statement: cite check는 v0에서 ADR-0008로부터 보존된 5종을 block한다 — stale, retracted, horizon mismatch, unit mismatch, overclaim. cite check block 시 ContentDraft는 Publication으로 승격되지 않는다 (ADR-0008 INV-0008-3 보존)
    status: active
  - id: INV-0015-6
    statement: cite check 6번째 block check 추가 — unresolved HIGH/CRITICAL access_intervention(status=pending_user_review, ADR-0018)을 핵심 근거로 인용 시 inline_block. severity LOW/MEDIUM은 hint로만 사용 가능 (R18). v0 block 총 6종 (5 + 1)
    status: active
  - id: INV-0015-7
    statement: cite check 7번째는 v1+ warning(block 아님) — thesis.stance/market_stance가 한쪽 방향으로 강하게 기울었는데 opposing_evidence/mitigating_factors/uncertainty_factors가 비어 있을 때 warn (R23/R25 bidirectional framing). v1+ block 6 + warning 1 = 7종
    status: active
  - id: INV-0015-8
...
preconditions:
  ...
  - id: PRE-0015-2
    statement: access_intervention severity 산정(ADR-0018) 후 6번째 block check(v0 5+1)가 활성화됨
  - id: PRE-0015-3
    statement: thesis.stance/market_stance + impact_targets (ADR-0019) 도입 후 v1+ warning check(block과 별개)가 활성화됨
...
# ADR-0015: Evidence (nullable + quote_reason + storage_level) and cite check 5+1 block + warning (v1+)
...
**6번째 block 추가 (R18, "5+1")**: **unresolved HIGH/CRITICAL access_intervention**
(ADR-0018)을 핵심 근거로 인용 시 inline_block. severity LOW/MEDIUM은 hint로만.
v0 block은 총 6종 (5 + access_intervention).

**Warning (R23/R25, v1+ 도입, block과 별개)**: **one-sided thesis warning** — thesis.stance
...
cite check 1~6 block(v0 5+1) 시 ContentDraft state를 reviewing으로 되돌린다.
v1+ warning은 block이 아니라 reviewer가 명시적으로 dismiss 가능.

## Alternatives Considered

- **A** (chosen): nullable quote + quote_reason 필수 + storage_level 4단계 +
  v0 6 block(=5 inherited + access_intervention) + v1+ warning(별개)
...
- 긍정:
  ...
  - 6번째 access_intervention block check (v0 5+1)로 publication core evidence가
    보장된 source에서만 나옴
  - v1+ warning(block과 별개)으로 bidirectional framing 자연스러운 enforce
...
- 후속 작업:
  ...
  - PUB-1A 단계: cite check 6종 block(v0 5+1) 구현 + v1+ warning(별개) 후속
  - Q-001 horizon enum 정의 후 horizon check enforce
  - ADR-0018 access_intervention severity 산정 활성화 후 6번째 block enforce
  - ADR-0019 thesis.stance/market_stance 활성화 후 v1+ warning enforce

## References

- ideation 출처: Round 3 결정 (9)(11) (ADR-0008 보존), Round 10/Q5 (quote
  nullable + quote_reason + storage_level), Round 11/Q6 (visual_policy —
  별도 ADR), Round 18 (6번째 access_intervention block "5+1"), Round 23/R25
  (v1+ bidirectional warning, block과 별개)
- Supersedes: ADR-0008
