 ...
 *
 * Credentials from env (Doppler — see docs/05_RUNBOOK.md "Doppler integration"):
 *   S3_ENDPOINT     — full S3 endpoint URL (preferred; supports custom domains,
 *                     S3-compatible mocks, non-Cloudflare backends)
 *   S3_ACCOUNT_ID   — Cloudflare account ID (fallback — endpoint auto-built as
...
function buildEndpoint(): string {
  // Prefer explicit S3_ENDPOINT — supports custom domains / S3-compatible
  // mocks / non-Cloudflare backends. Fall back to auto-built Cloudflare URL
  // from S3_ACCOUNT_ID for the common "vanilla R2" setup.
  const explicit = process.env["S3_ENDPOINT"];
  if (explicit) return explicit;
  const accountId = process.env["S3_ACCOUNT_ID"];
  if (!accountId) {
    throw new Error("S3_ENDPOINT or S3_ACCOUNT_ID env var must be set");
  }
...
export function r2CredentialsAvailable(): boolean {
  return Boolean(
    (process.env["S3_ENDPOINT"] || process.env["S3_ACCOUNT_ID"]) &&
